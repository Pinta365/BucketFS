...
/**
 * DigitalOcean Spaces storage plugin implementation.
 * Uses S3 REST API directly via fetch
 */
class DigitalOceanSpacesPlugin implements ProviderPlugin {
...
/**
 * Create a DigitalOcean Spaces storage plugin instance.
 * @param config Full bucket configuration
 * @returns A new DigitalOceanSpacesPlugin instance
 */
export function createPlugin(config: {
    ...
    if (!config.region) {
        throw new Error("Region is required for DigitalOcean Spaces provider");
    }
    const credentials = config.credentials as { accessKeyId?: string; secretAccessKey?: string } | undefined;
    if (!credentials?.accessKeyId || !credentials?.secretAccessKey) {
        throw new Error("Access key and secret key are required for DigitalOcean Spaces provider");
    }
    ...
    return Promise.resolve(new DigitalOceanSpacesPlugin(awsCredentials, endpointConfig));
}
